defmodule Chirp.Timeline do
  ...
  def inc_likes(id) do
    {1, [post]} =
      ...
      |> Repo.update_all(inc: [likes_count: 1], set: [updated_at: DateTime.utc_now()])
  ...
  def inc_reposts(id) do
    {1, [post]} =
      from(p in Post, where: p.id == ^id, select: p)
      |> Repo.update_all(inc: [repos_count: 1], set: [updated_at: DateTime.utc_now()])
    broadcast({:ok, post}, :post_updated)
  end
end
